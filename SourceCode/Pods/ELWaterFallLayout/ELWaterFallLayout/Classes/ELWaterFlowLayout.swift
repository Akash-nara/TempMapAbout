...
open class ELWaterFlowLayout: UICollectionViewFlowLayout {
    ...
    var arrOfPreviosHeight: [CGFloat] = []
    
    open var totalNumber = 0
    ...
    func findLastRowSeries(number: Int) -> Bool {
        var evenNumbers = [Int]()
        var firstNumber = 2
        for _ in 0...self.totalNumber {
            
            if firstNumber > self.totalNumber {
                break
            }
            evenNumbers.append(firstNumber)
            firstNumber = firstNumber + 3
        }
        return evenNumbers.contains(number)
    }
    
    func findSecondRowSeries(number: Int) -> Bool {
        var evenNumbers = [Int]()
        var firstNumber = 1
        for _ in 0...self.totalNumber {
        ...
        print("findSecondRowSeries", evenNumbers)
        return evenNumbers.contains(number)
    }
    ...
    func modifyLayoutAttributes(attr : UICollectionViewLayoutAttributes?) {
        if let a = attr {
            ...
            case .horizontal:
                let sl = self.findShortestLine()
                var height = self.lineWidth
                let x = sl.height + self.hItemSpace
                
                if a.indexPath.row == 1 || a.indexPath.row == 5 || a.indexPath.row == 6 || a.indexPath.row == 10 || a.indexPath.row == 12 || a.indexPath.row == 16 || a.indexPath.row == 20 {
                    height = (646 / 2)
...
//                if self.forGetMaxSize(number: a.indexPath.row) {
//                    height = 646 / 2
//
//                } else if self.forGetMinSize(number: a.indexPath.row) {
//                    height = (646 / 2) / 2
//
//                } else {
//                    height = (646 / 2) / 2
//                }
                ...
                self.arrOfPreviosHeight.insert(height, at: a.indexPath.row)

                var newF: CGFloat = 0
                
                if self.findLastRowSeries(number: a.indexPath.row) {
                    let yy = self.arrOfPreviosHeight[a.indexPath.row - 1]
                    let yy1 = self.arrOfPreviosHeight[a.indexPath.row - 2]
                    newF = yy + yy1
                    
                } else if self.findSecondRowSeries(number: a.indexPath.row) {
                    newF = self.arrOfPreviosHeight[a.indexPath.row - 1]
                    
                } else {
                    newF = 0
                }
                
                let y = (newF + self.vItemSpace) + self.edge.top
                let width = self.delegate?.el_flowLayout(self, heightForRowAt: a.indexPath.row)
                print("Final Y for \(a.indexPath.row) ", y)
                print("Final Height for \(a.indexPath.row) ", height)
                a.frame = CGRect(x: x, y: y, width: width ?? 0, height: height)
                self.updateLineHeightRec(at: sl.index, with: x + (width ?? 0))
