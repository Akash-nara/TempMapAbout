...
class PersonalDetailsViewModel {
    ...
    func getUserListAPI(param: [String: Any], success: @escaping ([String:JSON]?) -> Void, failure: @escaping (_ errorResponse: ()?) -> Void){
        
        API_SERVICES.callAPI(param, path: .getSearchUserList, method: .post) { responseJson in
            success(responseJson)
        } failure: { str in
            Utility.errorMessage(message: str ?? "Something went to wrong")
        }
